...
ggplot(filter(R2test_retour,no_trajet==20))+geom_sf(aes(color=nb_points))

ggplot(filter(R2test_retour,no_trajet==20))+geom_point(aes(x=id,y=nb_points,color=activity_plus))
...
gear.glm.plus.nb<-model.traj.glm(filter(R2test_retour,code_engin==engin_encours),observation="activity_plus",form= "dist+R2n+rel.angle+nb_points")
...
head(R2.pred.plus)
gear<-'FMC'
R2.pred.plus %>% dplyr::filter(activity_plus=='active' & substr(code_engin,1,3)==gear) %>% distinct(no_trajet)->labelled_traj

labelled_dta <- R2.pred.plus %>% dplyr::filter(no_trajet %in% labelled_traj$no_trajet)
liste_trajet <- unique(labelled_dta$no_trajet)

dta          <- R2.pred.plus %>% filter(substr(code_engin,1,3)==gear)

colX    <- which(stringr::str_detect(colnames(labelled_dta) , 'speed|circle|conv'))
formula_train <- paste('etat_agrege ~ ', paste(colnames(labelled_dta)[colX],  collapse = ' + ') )
count_class <-  labelled_dta %>% dplyr::group_by(etat_agrege) %>% dplyr::summarise(count = n()) %>% dplyr::select(count) %>% unlist()
weight_class <-  round(sum(count_class) / count_class,1)
mod.RF    <- ranger(formula_train,
                    data = labelled_dta, num.trees = 800, mtry = 6, class.weights = weight_class)
predict_data   <- predict(mod.RF, data = dta)

dta %>% dplyr::mutate(etat_predit = predict_data$predictions,Gear =gear,methode='RF')  -> res_FMC

transfert <- res_FMC %>% dplyr::select(idpos, methode,etat_predit)
