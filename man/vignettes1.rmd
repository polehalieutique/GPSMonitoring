...
R2 %>% filter(code_engin==engin_encours) %>% 
...
liste_trajet_silico<-c(1,11,6,7,20)
...
R2test_retour<-R2 %>% inner_join(do.call(rbind,lapply(split(R2, R2$no_trajet),add.nb.point,r=2000)))


ggplot(filter(R2test_retour,no_trajet==29))+geom_sf(aes(color=nb_points))

ggplot(filter(R2test_retour,no_trajet==160))+geom_point(aes(x=id,y=nb_points,color=activity_plus))
...
#					   from communes_uni")
grid<-st_read(con,query="select *  from cells")
head(grid)
ggplot(grid)+geom_sf(color=NA)+geom_sf(data=R2_engin)

 st_join(grid,filter(R2_engin,predict_glm==TRUE),left=FALSE) %>% group_by(code_engin,id) %>% dplyr::summarize(total=n()) %>% 
